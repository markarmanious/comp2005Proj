...
def init_subs():
        db = get_db()
        db.execute('create table subscriptions (id integer primary key autoincrement, userid integer not null, topicid integer not null, viewed boolean not null);')
        db.commit()

@app.cli.command('initsubs')
def initsubs_command():
        init_subs()
        print('Initialized the subscription database.')
